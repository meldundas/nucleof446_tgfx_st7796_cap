...
extern "C" {
    void updateUartData_C(char* data, uint16_t size);
}
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, &single_byte, 1);
  /* USER CODE END 2 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        if(uart_buf_idx < UART_BUFFER_SIZE -1)
        {
            uart_buf[uart_buf_idx++] = single_byte;
        }

        if(single_byte == '\n' || single_byte == '\r')
        {
            updateUartData_C((char*)uart_buf, uart_buf_idx);
            uart_buf_idx = 0;
